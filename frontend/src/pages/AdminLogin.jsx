...
import { ShieldCheck, ArrowRight, Loader2, Home, Lock, Eye, EyeOff } from "lucide-react";
...
export default function AdminLogin() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans flex flex-col relative isolate overflow-hidden">
      {/* Ambient Background */}
      ...
      {/* Top Nav */}
      <nav className="relative px-6 sm:px-10 py-5 flex justify-between items-center border-b border-white/5">
        ...
        <div className="flex items-center gap-2 text-white/20">
          ...
          <span className="text-xs font-semibold uppercase tracking-widest">Restricted Access</span>
        </div>
      </nav>
          ...
          <div className="bg-white/[0.04] backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl shadow-black/60">
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-white/50 uppercase tracking-widest pl-1">Admin User ID</label>
                <Input
                  id="admin-user-id"
                  value={adminId}
                  ...
                  className="h-12 bg-black/30 border-white/10 focus-visible:border-rose-500/50 focus-visible:ring-rose-500/20 rounded-xl text-white placeholder:text-white/20"
                  disabled={isLoading}
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Input
                    ...
                    className="h-12 bg-black/30 border-white/10 focus-visible:border-rose-500/50 focus-visible:ring-rose-500/20 rounded-xl text-white placeholder:text-white/20 pr-11"
                    disabled={isLoading}
                  />
                  <button
                    type="button"
              ...
              <Button
                id="admin-login-btn"
                type="submit"
                className="w-full h-12 bg-rose-600 hover:bg-rose-500 text-white font-bold text-sm rounded-xl shadow-lg shadow-rose-900/30 mt-2 group transition-all"
                disabled={isLoading}
              >
                {isLoading ? (
