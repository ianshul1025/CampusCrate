    ...
    <div className="space-y-7">
      ...
      <div>
        ...
        <Tabs value={activeStatus} onValueChange={setActiveStatus} className="w-full">
          <TabsList className="grid grid-cols-3 bg-card border border-border p-1 h-auto rounded-lg">
            <TabsTrigger value="all" className="rounded-md data-[state=active]:bg-primary data-[state=active]:text-foreground font-semibold text-xs">All</TabsTrigger>
            <TabsTrigger value="Lost" className="rounded-md data-[state=active]:bg-primary data-[state=active]:text-foreground font-semibold text-xs">Lost</TabsTrigger>
            <TabsTrigger value="Found" className="rounded-md data-[state=active]:bg-green-600 data-[state=active]:text-foreground font-semibold text-xs">Found</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
    </div>
    ...
    <div className="h-screen overflow-hidden bg-background text-foreground flex flex-col font-sans selection:bg-primary/30">
          ...
          <div className="p-6 md:p-8 lg:p-10 max-w-7xl mx-auto">
            ...
            <div className="mb-8">
              <div className="flex items-center text-sm text-muted-foreground mb-4 gap-2 font-medium">
                ...
                <span className="text-foreground">Item Feed</span>
              </div>
              ...
              <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-6 mb-6">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Button
                      ...
                      className="md:hidden mb-2 border-border bg-secondary hover:bg-secondary text-foreground rounded-lg"
                    >
                ...
                <div className="flex items-center gap-4">
                  <Select value={sortOrder} onValueChange={setSortOrder}>
                    <SelectTrigger className="w-[190px] bg-secondary border-border h-10 rounded-lg">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            {loading ? (
              ...
              <div className="bg-card/20 border border-border p-16 rounded-3xl text-center flex flex-col items-center justify-center">
                ...
                <Button onClick={resetFilters} variant="outline" className="border-border rounded-full px-8 bg-transparent text-foreground font-semibold">
                  Clear Filters
                </Button>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredItems.map(item => {
                  ...
                  return (
                    <Card key={item._id} className={`bg-card border-border transition-all duration-300 shadow-lg rounded-2xl h-full flex flex-col overflow-hidden relative isolate ${isReturned ? "opacity-60 grayscale-[50%]" : "hover:border-border hover:bg-card/80 group hover:-translate-y-1"}`}>
                      ...
                      {/* Image Area */}
                      <Link to={isReturned ? "#" : `/item/${item._id}`} className={`block h-52 bg-secondary relative overflow-hidden flex-shrink-0 ${isReturned ? "pointer-events-none" : ""}`}>
                        {item.imageUrl ? (
                      ...
                      <CardContent className="p-5 flex-1 flex flex-col relative z-20">
                        ...
                        {/* Action Buttons */}
                        <div className="mt-auto pt-4 border-t border-border grid gap-3">
                          {isReturned ? (
                            <Button 
                              disabled 
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <Link to={`/item/${item._id}`} className="w-full">
                                <Button variant="outline" size="sm" className="w-full h-9 border-border bg-secondary hover:bg-secondary rounded-xl text-xs font-bold gap-1.5">
                              ...
                              {!isOwner ? (
                                existingClaim ? (
                                  <Button 
                                    variant="ghost" 
                                    size="sm" 
                                    className="w-full h-9 bg-secondary text-muted-foreground text-[10px] items-center justify-center text-center px-1 leading-tight rounded-xl border border-dashed border-border"
                                  ...
                                  <Link to={`/item/${item._id}?claim=true`} className="w-full">
                                    <Button 
                                      ...
                                      className={`w-full h-9 rounded-xl text-xs font-black gap-1.5 shadow-sm active:scale-95 transition-transform ${
                                        isLost 
                                          ? "bg-red-600 hover:bg-red-500 text-primary-foreground" 
      ...
      {mobileFiltersOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          />
          <aside className="absolute right-0 top-0 h-full w-[88vw] max-w-sm border-l border-border bg-background p-5 overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Item Feed Filters</h3>
              <button
                onClick={() => setMobileFiltersOpen(false)}
                className="h-8 w-8 rounded-lg border border-border bg-secondary flex items-center justify-center text-muted-foreground hover:text-foreground"
                aria-label="Close filters"
              >
                <X className="h-4 w-4" />
              </button>
